...
function main() {
   ...
   */
  function createVerboseProgress(): (progress: {
    phase: string;
    current: number;
    total: number;
    currentFile?: string;
  ...
  function printIndexResult(
    ...
    if (hasErrors) {
      ...
      if (result.filesIndexed > 0) {
        clack.log.info(
          `The index is fully usable ${getGlyphs().dash} only the failed files are missing.`,
        );
      }
    } else if (projectPath) {
      ...
      async (
        ...
        try {
          if (isInitialized(projectPath)) {
            ...
            try {
              //TODO: offerWatchFallback 已初始化完成
  ...
  program
    ...
    .action(
      async (pathArg: string | undefined, options: { json?: boolean }) => {
        ...
        const worktreeMismatch = detectWorktreeIndexMismatch(
          startPath,
          projectPath,
        );
            ...
            console.log(
              JSON.stringify({
                ...
                worktreeMismatch: worktreeMismatch
                  ? {
                      worktreeRoot: worktreeMismatch.worktreeRoot,
                      indexRoot: worktreeMismatch.indexRoot,
                    }
                  : null,
              }),
            );
      ...
      async (
        ...
        try {
          ...
          const { isGeneratedFile } =
            await import("../extraction/generated-detection");
...
`),
        ...
        try {
          ...
          if (options.json) {
            ...
            for (const node of limited) {
              ...
              console.log(
                chalk.cyan(node.kind.padEnd(12)) + chalk.white(node.name),
              );
          ...
          error(
            `callers failed: ${err instanceof Error ? err.message : String(err)}`,
          );
          process.exit(1);
        }
      },
    ...
    .action(
      async (
        ...
        try {
          ...
          error(
            `callees failed: ${err instanceof Error ? err.message : String(err)}`,
          );
          process.exit(1);
        }
      },
  ...
  program
    ...
    .action(
      async (
        symbol: string,
        options: { path?: string; depth?: string; json?: boolean },
      ) => {
        const projectPath = resolveProjectPath(options.path);
        ...
        try {
          ...
          const depth = Math.min(
            Math.max(parseInt(options.depth || "2", 10), 1),
            10,
          );
          ...
          if (options.json) {
            ...
            const byFile = new Map<
              string,
              Array<{ name: string; kind: string; startLine?: number }>
            >();
            ...
            for (const [file, nodes] of byFile) {
              ...
              for (const node of nodes) {
                const loc = node.startLine ? `:${node.startLine}` : "";
                console.log(
                  `  ${chalk.dim(node.kind.padEnd(12))}${node.name}${chalk.dim(loc)}`,
                );
              }
              console.log();
            }
      ...
      async (opts: {
        ...
        if (opts.printConfig) {
          const { getTarget, listTargetIds } =
            await import("../installer/targets/registry");
          ...
          const loc = (opts.location === "local" ? "local" : "global") as
            | "global"
            | "local";
  ...
  program
    ...
    .action(
      async (opts: { target?: string; location?: string; yes?: boolean }) => {
